...
public class AnyDOIProviderFactory implements PidProviderFactory {
    ...
    public PidProvider createPidProvider(String providerId) {
        ...
        if (!providerType.equals(AnyDOIProvider.TYPE)) {
            return null;
        }
        // common settings
        ...
        String anyDoiUrl = JvmSettings.ANYDOI_URL.lookupOptional(providerId).orElse("http://localhost:8520");
        return new AnyDOIProvider(providerId, providerLabel, providerAuthority, providerShoulder, identifierGenerationStyle,
                datafilePidFormat, managedList, excludedList, anyDoiUrl);
    }

    public String getType() {
        return AnyDOIProvider.TYPE;
    }
}
